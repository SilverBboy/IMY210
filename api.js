...
app.get('/faqs', async (request, result) => {
	try {
...
app.post('/faqs/:code', async (request, result) => {
	try {
		const data = await accessXDB();
		const faqs = data.faqs.module;
...
app.get('/faqs/:code', async (request, result) => {
	try {
		const data = await accessXDB();
		const faqs = data.faqs.module;
		// const module = faqs.find((item) => item.code === request.params.code);
...
app.put('/faqs/:code/:priority', async (request, result) => {
	try {
		const data = await accessXDB();
		const faqs = data.faqs.module;
		// const module = faqs.find((item) => item.code === request.params.code);
...
app.delete('/faqs/:code/:priority', async (request, result) => {
	try {
		const data = await accessXDB();
		const faqs = data.faqs.module;
		const module = retrieveModule(faqs, request.params.code);
